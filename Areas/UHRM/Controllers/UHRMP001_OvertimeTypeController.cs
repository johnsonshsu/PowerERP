...
namespace powererp.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("UHRM")]
    public class UHRMP001_OvertimeTypeController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public UHRMP001_OvertimeTypeController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public IActionResult Init(string id = "", int initPage = 1)
        {
            ...
            {
                var sqlData = new z_sqlCodeDatas();
                SessionService.BaseNo = sqlData.GetDataList()
                .Where(m => m.BaseNo == "Overtime")
                .OrderBy(m => m.SortNo)
                .FirstOrDefault().CodeNo;
            }
            //這裏可以寫入初始程式
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new z_sqlOvertimeTypes();
            ...
            SetIndexViewBag();
            //設定主檔資料列表
            using var sqlCodeDatas = new z_sqlCodeDatas();
            var masterList = sqlCodeDatas.GetDropDownList("Overtime", true);
        ...
        [Login(RoleList = "User")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(OvertimeTypes model)
        {
            ...
            if (model.Id == 0)
            {
                model.BaseNo = Guid.NewGuid().ToString();
            }
        ...
        {
            using var sqlData = new z_sqlOvertimes(); return sqlData.Delete(id);
        }
    }
}